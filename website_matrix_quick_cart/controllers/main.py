...
class website_matrix_quick_cart_view(WebsiteSale):
    def get_attribute_value_ids(self, product, list=False):
        ...
        if request.website.pricelist_id.id != pricelist.id:
            ...
            attribute_value_ids = [[p.id, [v.id for v in p.product_template_attribute_value_ids if v.attribute_id.id in visible_attrs], p.price, p.lst_price] for p in product.product_variant_ids]
